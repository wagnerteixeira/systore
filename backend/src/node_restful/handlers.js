  ...
  if (req.params.id && !mongoose.Types.ObjectId.isValid(req.params.id)) {
    ...
    req.quer.exec(function(err, list) {
      if (err) {
        ...
        exports.respondOrErr(
          res,
          404,
          !list && exports.objectNotFound(),
          200,
          list && _.isArray(list) ? list[0] : list
        );
      } else {
...
exports.put = function(req, res, next) {
  ...
  if (this.shouldUseAtomicUpdate) {
    req.quer.findOneAndUpdate({}, req.body, this.update_options, function(
      err,
      newObj
    ) {
      if (err) {
    ...
    req.quer.findOne({ _id: req.params.id }, function(err, docToUpdate) {
      ...
      docToUpdate['$__']._user = req.body._user;
...
exports.delete = function(req, res, next) {
  ...
  if (this.shouldUseAtomicUpdate) {
    ...
    req.quer.findOne({ _id: req.params.id }, function(err, docToRemove) {
      ...
      docToRemove['$__']._user = req.body._user;
