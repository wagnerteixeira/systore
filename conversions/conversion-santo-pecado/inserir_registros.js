...
const inserirRegistros = async () => {
  ...
  let arrayFileClient = fs.readFileSync(path_clientes).toString().split("\n");  
  ...
  let arrayFilebillsReceive = fs.readFileSync(path_titulos).toString().split("\n");  
  for(i in arrayFilebillsReceive) { 
    ...
    if (client) {      
      ...
      {
        problems.push(`Não foi possível inserir o título ${line[1]} - ${line[2]}`);
        console.log(e.response.data.errors);
        return;
      }
    }    
    else 
    {
      ...
      problems.push(`Cliente ${line[0]} não encontrado , não foi possível inserir o título ${line[1]} - ${line[2]}`);
    }
